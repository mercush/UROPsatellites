function [avg_pass, avg_coverage, avg_interval] = STK_access_radar(iteration,TLE)
...
MeanMotion = TLE.GetMeanMotion(n);
Eccentricity = TLE.GetEccentricity(n);
Inclination = TLE.GetInclination(n);
ArgOfPerigee = TLE.GetArgPerigee(n);
RAAN = TLE.GetRAAN(n);
MeanAnomaly = TLE.GetMeanAnomaly(n);
...
avg_pass = totalaccess(n)/no_intervals(1);
avg_coverage = totalaccess(n)/2592000;
avg_interval = (2592000-totalaccess(n))/no_intervals(1);
